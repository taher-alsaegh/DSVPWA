...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="DSVPWA", description="Damn Simple Vulnerable Python Web Application"
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
    ...
    # --- ONLY CHANGE: robust PORT parsing for Kubernetes ---
    raw_port = os.getenv("DSVPWA_PORT", "65413")
    if isinstance(raw_port, str) and raw_port.startswith("tcp://"):
        raw_port = raw_port.rsplit(":", 1)[-1]

    parser.add_argument(
        "--port",
        type=int,
        default=int(raw_port),
    ...
    parser.add_argument(
        "--risk",
        type=int,
        default=os.getenv("DSVPWA_RISK", 1),
        choices=range(1, 4),
    ...
    parser.add_argument(
        "--ssl",
        action="store_true",
        default=os.getenv("DSVPWA_SSL", 0),
    ...
    try:
        ...
        print(
            "[*] Navigate to {}://{}:{} to access DSVPWA".format(
